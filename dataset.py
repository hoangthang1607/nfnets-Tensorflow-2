...
def load(
    ...
    if name.lower() == "imagenet":
        ...
        ds = tfds.load(
            "imagenet2012:5.*.*",
            split=tfds_split,
            data_dir="~/tensorflow_datasets/downloads",
            download_and_prepare_kwargs={
                "download_dir": "~/tensorflow_datasets/downloads"
            },
            shuffle_files=True,
    ...
    def preprocess(example):
        image = _preprocess_image(
            example["image"], is_training, image_size, eval_preproc
        )
    ...
    def cast_fn(batch):
        batch = dict(**batch)
        batch["images"] = tf.cast(batch["images"], dtype)
        return batch
        ...
        if i == 0:
            ...
            if augment_name is not None:
                ...
                if not augment_before_mix and (
                    ...
                    ds = ds.unbatch().map(
                        augment_normalize, num_parallel_calls=AUTOTUNE
                    )
                    ds = ds.batch(batch_size)
            else:
                ds = ds.batch(batch_size)
...
def _preprocess_image(
    ...
    if is_training:
        ...
        if eval_preproc == "crop_resize":
            ...
            image = _decode_and_resize_then_crop(
                image_bytes, image_size=image_size, crop_pct=crop_pct
            )
        else:
...
def crop(image_bytes, crop_window):
    ...
    if image_bytes.dtype == tf.dtypes.string:
        image = tf.image.decode_and_crop_jpeg(
            image_bytes, tf.stack(crop_window), channels=3
        )
    else:
...
def my_mixup_cutmix(batch):
    ...
    cutmix_images = (
        cutmix_mask * batch["images"][2 * bs : 3 * bs]
    ...
    return {
        "images": tf.concat([mixup_images, cutmix_images], axis=0),
